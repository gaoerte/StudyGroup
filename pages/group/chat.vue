<template>
  <view class="container">
    ...
    <view class="input-area">
      ...
      <button @click="sendMessage" :disabled="!socketOpen || !message.trim()" class="send-button">发送</button>
    </view>
  </view>
</template>
...
export default {
  ...
  onLoad(options) {
    ...
    // 使用 SocketTask 连接 WebSocket
    this.socket = uni.connectSocket({
      url: `ws://172.25.126.124:8081/group/${this.groupId}`,  // 使用 groupId 创建房间
    });

    // 监听 WebSocket 连接打开事件
    uni.onSocketOpen(() => {
      console.log('WebSocket 连接成功');
      this.socketOpen = true;  // 连接成功后启用按钮
    });

    // 监听 WebSocket 消息事件
    uni.onSocketMessage((res) => {
      try {
        const message = JSON.parse(res.data); // 确保消息是有效的 JSON 格式
